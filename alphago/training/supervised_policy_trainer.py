...
class threading_shuffled_hdf5_batch_generator:
    ...
    def shuffle_indices(self, seed=None, idx=0):
        ...
        if seed is None:
            # create random seed
            self.metadata['generator_seed'] = random.randrange(4294967295)

        # feed numpy.random with seed in order to continue with certain batch
        random.seed(self.metadata['generator_seed'])
...
def validate_feature_planes(verbose, dataset, model_features):
    ...
    if 'features' in dataset:
        dataset_features = dataset['features'][()].decode('ascii').split(',')
...
def start_training(args):
    ...
    weight_folder = args.out_directory
