...
class threading_shuffled_hdf5_batch_generator:
    ...
    def shuffle_indices(self, seed=None, idx=0):
        ...
        if seed is None:
            ...
            self.metadata['generator_seed'] = np.random.randint(4294967295)
...
def validate_feature_planes(verbose, dataset, model_features):
    ...
    if 'features' in dataset:
        ...
        if n_dataset_planes != n_model_planes:
            raise ValueError(
                "Model JSON file expects a total of {} planes from features \n\t{}\n"
                "But dataset contains {} planes".format(
                    n_model_planes,
                    "\n\t".join(model_features),
                    n_dataset_planes))
...
def create_and_save_shuffle_indices(train_val_test, max_validation,
    ...
    n_val_data = math.ceil(train_val_test[1] * len(shuffle_indices))
    ...
    n_test_data = math.ceil(train_val_test[2] * len(shuffle_indices))
...
def train(metadata, out_directory, verbose, weight_file, meta_file):
    ...
    if resume:
        ...
        model.load_weights(os.path.join(out_directory, weight_file))
    ...
    save_acc_graph = get_graph_saver(filename=os.path.join(out_directory, "accuracy.png"),
                                     title="Model accuracy", xlabel='Epoch', ylabel='Accuracy')
    save_acc_graph(training=hist.history['acc'], test=hist.history['val_acc'])

    save_loss_graph = get_graph_saver(filename=os.path.join(out_directory, "loss.png"),
                                      title="Model loss", xlabel='Epoch', ylabel='Loss')
    save_loss_graph(training=hist.history['loss'], test=hist.history['val_loss'])
...
def start_training(args):
    ...
    weight_folder = os.path.join(args.out_directory)
...
def resume_training(args):
    ...
    if args.verbose:
        ...
        print("trying to resume training from %s with weights %s" %
              (meta_file, os.path.join(args.out_directory, weight_file)))
