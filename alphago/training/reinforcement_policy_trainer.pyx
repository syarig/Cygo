...
def run_n_games(optimizer, lr, learner, opponent, num_games, mock_states=None):
    ...
    odd_states = states[1::2]   # state[start:end:interval]
...
def run_training(cmd_line_args=None):
    ...
    for i_iter in range(1, args.iterations + 1):
        ...
        if i_iter % args.record_every == 0:
            player_weights = "weights.{:05d}.hdf5".format(i_iter)
