...
cdef dict HASH_LOOKUP = {
    WHITE: [[random.randint(0, np.iinfo(np.uint64).max) for i in range(19)] for i in range(19)],
    EMPTY: [[random.randint(0, np.iinfo(np.uint64).max) for i in range(19)] for i in range(19)],
    BLACK: [[random.randint(0, np.iinfo(np.uint64).max) for i in range(19)] for i in range(19)]
}
...
cdef class GameState:
    ...
    def __init__(self, size=19, komi=7.5, enforce_superko=False):
        ...
        self.check_board = np.zeros((size, size), dtype=np.bool)
        self.size = size
        ...
        for x in range(size):
            for y in range(size):
                self.liberty_sets[x][y] = set(self.get_neighbors((x, y)))
        # separately cache the 2D numpy array of the _size_ of liberty sets
        # at each board position
        self.liberty_counts = np.zeros((size, size), dtype=np.int32)
    ...
    def __getstate__(self):
        return {
            ...
            'komi': self.komi,
            'board': self.board,
            'es': self.enforce_superko,
            'cp': self.current_player,
            'ko': self.ko,
            'handicaps': self.handicaps,
            'history': self.history,
            'num_bp': self.num_black_prisoners,
            'num_wp': self.num_white_prisoners,
            'current_hash': self.current_hash,
            'previous_hashes': self.previous_hashes,
            'group_sets': self.group_sets,
            'liberty_sets': self.liberty_sets,
            'liberty_counts': self.liberty_counts,
            'stone_ages': self.stone_ages,
        }
    ...
    def __setstate__(self, state):
        ...
        self.current_player = state['cp']
        self.ko = state['ko']
        self.handicaps = list(state['handicaps'])
        self.history = list(state['history'])
        self.num_black_prisoners = state['num_bp']
        self.num_white_prisoners = state['num_wp']
        self.current_hash = state['current_hash']
        self.previous_hashes = state['previous_hashes']
        self.group_sets = state['group_sets']
        self.liberty_sets = state['liberty_sets']
        self.liberty_counts = state['liberty_counts']
        self.stone_ages = state['stone_ages']
        self.is_end_of_game = False
        self.__legal_move_cache = None
        self.__legal_eyes_cache = None
        self._create_neighbors_cache()
    ...
    cpdef list get_groups_around(self, tuple position):
        ...
        """
        groups = []
        for (nx, ny) in self.get_neighbors(position):
    ...
    def get_diagonals(self, position):
        ...
        return [(nx, ny) for nx, ny in [(x - 1, y - 1), (x + 1, y + 1), (x + 1, y - 1), (x - 1, y + 1)]
                if self.on_board(nx, ny)]
    ...
    def get_current_hash(self):
        ch = self.current_hash
        if self.ko != None:
            x, y = self.ko
            ch ^= HASH_LOOKUP[EMPTY][x][y]

        return ch, self.current_player, len(self.history)

    def _update_hash(self, action, color):
        (x, y) = action
        self.current_hash ^= HASH_LOOKUP[color][x][y]
    ...
    def is_suicide(self, action):
        ...
        if num_liberties_here == 0:
            ...
            for (nx, ny) in self.get_neighbors(action):
                ...
                group_has_other_liberties = len(self.liberty_sets[nx][ny] - {action}) > 0
    ...
    cpdef bint is_eye(self, tuple position, long owner, object stack=None):
        ...
        cdef:
            ...
            long num_bad_diagonal = 0
            # if in middle of board, 1 bad neighbor is allowable; zero for edges and corners
            long allowable_bad_diagonal = 1 if len(self.get_neighbors(position)) == 4 else 0
            tuple d
        ...
        for d in self.get_diagonals(position):
            ...
            elif self.board[d] == _EMPTY and d not in stack:
    ...
    def is_ladder_capture(self, action, prey=None, remaining_attempts=80):
        ...
        for (prey_x, prey_y) in potential_prey:
            ...
            for prey_stone in tmp.group_sets[prey_x][prey_y]:
                for (nx, ny) in tmp.get_neighbors(prey_stone):
    ...
    cpdef list get_legal_moves(self, bint include_eyes=True):
    ...
    def do_move(self, action, color=None):
        ...
        if len(self.history) > 1:
            if self.history[-1] is PASS and self.history[-2] is PASS \
                    and self.current_player == WHITE:
                self.is_end_of_game = True
        return self.is_end_of_game


    def print_board(self, error=False):
        strBoard = []
        strBoard.append("   ")
        line_char = ' A B C D E F G H J K L M N O P Q R S T\n'
        strBoard.append(line_char)
        # lNumbers = ["%02d" % n for n in range(0, 19)]
        # strBoard.append(" ".join(lNumbers))
        # strBoard.append("\n")  # strBoard = "   00 01 02 03 04 05 06 07 08 09 10 11 12 13 14 15 16 17 18\n"

        if len(self.history):
            pre_action = self.history[-1]
        ...
        rot_board = np.rot90(self.board, 1)

        for x, line in enumerate(rot_board):
            strBoard.append("{:2d}: ".format(nLine))
            nLine -= 1
            for y, stone in enumerate(line):
                if stone == EMPTY:
                    strBoard.append(print_emp)
                elif stone == BLACK:
                    if pre_action == (x, y):
                        strBoard.append(print_pre_blk)
                    else:
                        strBoard.append(print_blk)
                elif stone == WHITE:
                    if pre_action == (x, y):
                        strBoard.append(print_pre_wht)
                    else:
                        strBoard.append(print_wht)
                else:
                    strBoard.append(print_none)
